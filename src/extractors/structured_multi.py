# File: src/extractors/structured_multi.py
...
# --- Heading Extraction from Heading Pages ---
def extract_from_heading_page(lines, page_num):
    ...
    for line in lines:
        ...
        # --- Multi-language heading leniency ---
        script = detect_script(text)
        if script in {"devanagari", "telugu"}:
            # allow single-line headings even if short
            min_words = 1
        else:
            min_words = 2
        ...
        if number_match:
            ...
            if trailing_text:
                ...
                if word_count >= min_words and line["size"] >= dominant_size:
        ...
        if is_heading_like(text, font=line["font"], size=line["size"]):
            if line["size"] >= dominant_size and len(text.split()) >= min_words:
                result.append({"level": "H1", "text": text.strip(), "page": page_num})

    return result

# --- Other Section Extractors ---
